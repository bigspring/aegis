...
class Navigation {
    ...
    public function build( $url, $title = NULL, $params = array() ){
    	...
    	if( is_array($url) ){
    		...
    		foreach( $url as $u ){
                ...
                if( !isset($u['activechildren']) ){
                	$u['activechildren'] = array();
                }
                
    			/** invoke the builder  */
    			$built[] = $this->_build( $u['url'], $u['title'], $u['params'], $u['children'], $u['activechildren'] );
    		}
     ...
     * @param  array $children 			child nav items
     * @param  array $hiddenchildren 	child nav items that should not be rendered, but used for active highlighting
     * @return string         			returns compiled navigation
     */
    private function _build( $url, $title, $params, $children, $activechildren ){

    	// check if the current URL matches the values in the hiddenchildren array   	
    	$uri = $this->ci->uri->segment_array();
    	
    	// see if we have a two part uri and generate a string to compare
    	// we do this to remove any query parameters
    	if($uri)
        {
        ...
        /** if the passed url is equal to the current uri_string or equal to one of the active children, set this li active */
        if( $url == $this->ci->uri->uri_string() || $url == current_url() || in_array($uri, $activechildren)){
