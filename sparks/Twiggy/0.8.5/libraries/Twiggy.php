...
class Twiggy
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->_twig->getExtension('core')->setDateFormat('d/m/Y', '%d days');
	...
	*/
	...
	private function _set_template_locations($theme)
	{
		...
		{
			// 2014-04-28 added class reference to permit full module paths in app/view dir
			$this->_module = $this->CI->router->fetch_module() . '/' . $this->CI->router->class;
			...
			if(!empty($this->_module))
			{
				...
				foreach($module_locations as $loc => $offset)
				{
					...
					if (is_dir($this->_themes_base_dir . $this->_module . '/'))
						$this->_template_locations[] = $this->_themes_base_dir . $this->_module . '/';
				}
			}
		}
